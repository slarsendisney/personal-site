---
title: "Hack Sussex 19"
path: /decks/SUSSEXHACK19
desc: American Express & Surviving Your First Hack.
hero: "./hero.png"
location: Brighton | Sussex University
---

import PresImage from '../../src/components/PresentationComponents/PresImage';
import Controls from '../../src/components/PresentationComponents/PresLayout';
import Preload from "../../src/components/PresentationComponents/Preload";

import {Engineers, SOS, Esc, AmericanExpress, Brighton, London, FirstHack, IHatePowerpoint, TheTeam,Challenge} from '../../src/components/PresentationComponents/gifs';

<Preload />
<Controls />


<PresImage image={AmericanExpress}/>

---
# 👋

---

# We'll keep this quick.

(We know why you're here!)

---

<PresImage image={Challenge}/>

---

# 💪

## Good Citzenship. 

---

# ❤️

### We want to ask you to build community tools. 

---

# 🏆

### Cool but whats the prize?!

---

# 💡

### Ideas to get you thinking

---

<PresImage image={FirstHack}/>

---

<PresImage image={SOS}/>

<h4 className="is-orange margin-0">sos.sld.codes</h4>

---

<PresImage image={TheTeam}/>

---

<PresImage image={Engineers}/>

---

<div class="container">

<h1>React, HTML, JS, Node, CSS, SCSS, Python, Java, Spring & Android</h1>

</div>

---

# 🕐 24hrs

---

<PresImage image={Esc}/>

---

# 🎮 Nintendo Switch

---

# 🎓 Scheme + ☀️ Internships

---

<PresImage image={Brighton}/>

---

<PresImage image={London}/>

---

# 💵 Competitive

---

# 🛂 Sponsor Visas

---

# 💬 Talk To Us

---

<Controls />

# Good luck!

<h4> Find us on the stand or find me at <span className="is-pink"> sld.codes </span> </h4>
